...
def plan_trip(starting_station, ending_station, trip_planner, stations, lines):
    ...
    elif starting_station in trip_planner and len(stations[starting_station]) <= 1:
    ...
    for line in stations[starting_station]:
        ...
        if counter == len(stations[starting_station])*2 and ending_station not in trip_planner:
            # print(len(stations[starting_ston]))
            # print(counter)
            trip_planner.remove(starting_station)
            # print(lines, "1")
            lines.remove(lines[-1])
    ...
    for i in range(len(lines) - 1):
        if i != len(lines):
            if lines[i] == lines[i + 1] and cnt == 0:
                ...
                print(f"At {trip_planner[i + 1]} transfer from the {lines[i]} line to the {lines[i + 1]} "
                      f"line.", end=" ")
        else:
...
def get_train_info(train_id, stations):
    pass
...
def metro_station(system):
    ...
    while user_input != "exit":
        ...
        if user_split[0] == "get" and user_split[1] == "train" and user_split[2] == "info":
            get_train_info(user_split[3], stations)
...
if __name__ == '__main__':
    # stations = {'Greenbelt': [{'Green': ['Greenbelt', 'College-Park']}], 'College-Park': [{'Green': ['College-Park', 'Greenbelt']}, {'Green': ['College-Park', 'Fort-Totten']}], 'Fort-Totten': [{'Green': ['Fort-Totten', 'College-Park']}]}
